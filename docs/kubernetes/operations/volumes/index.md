...
covers what is operated on an existing volume: moving its backing logical volume between storage nodes with a
`PersistentVolumeOps` operation, reclaiming the blocks its filesystem no longer uses, and getting a workload back to
work after its NVMe-oF paths were gone.
