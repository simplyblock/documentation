...
and the cluster rebalances afterward. This is the procedure for a host that is being replaced, decommissioned, or taken
out of the storage plane while its storage node lives on.

1. Bring the replacement worker into the Kubernetes cluster and confirm that it is `Ready`. The devices it offers have
   to match what the node expects, either at the same PCIe addresses as on the old host, or declared through
   `spec.migrate.newSsdPcie`.
2. Create a `StorageNodeOps` with the action `Migrate` and the new worker as `spec.migrate.targetWorkerNode`.
3. Follow the operation to `Succeeded`.

```yaml title="Example of relocating a storage node onto a replacement worker"
...
new host, promotes it, and re-points the Kubernetes topology onto the new worker. The details of each step, and what to
check when one of them stalls, are in [Migrating a Storage Node](migrating-a-storage-node.md).
...
The order matters. Adding the replacement first keeps the capacity and the redundancy of the cluster intact throughout,
which is why it is the recommended sequence.

1. **Add the replacement node** with a growth `ClusterDeploymentConfig` that names the cluster in `spec.clusterRef` and
   lists the new worker, as described in [Expanding a Storage Cluster](../scaling/expanding-storage-cluster.md).
2. **Wait for the new node** to reach the phase `Online`.
3. **Drain and remove the old node** with a `StorageNodeOps` of the action `Remove`. Its volumes are migrated onto the
   remaining nodes, including the one just added, before the node leaves the cluster. See
   [Removing a Storage Node](removing-a-storage-node.md).
4. **Verify** that the old node is gone and every volume is accounted for.

```yaml title="Example of removing the retired node once the replacement is online"
apiVersion: storage.simplyblock.io/v1alpha2
kind: StorageNodeOps
metadata:
...
A removal is blocked while any volume on the node is pinned to it, or while any volume has no `PersistentVolume`. Both
are reported with `DrainBlocked` events, and both have to be resolved before the drain proceeds.
...
replacement carries the same domain label as the node it replaces.

Relocating a node is the simpler option here as well, since the node keeps its domain membership. A retired node that
is replaced by a new one needs the domain set on the group of the growth document, as described in
[Managing Failure Domains](../cluster/failure-domains.md#assigning-storage-nodes-to-a-domain).
