...
On plain Linux (Docker-based deployments), these backups are managed with the `{{ cliname }} db-backup` commands
...
{{ cliname }} db-backup config <CLUSTER_ID> \
...
{{ cliname }} db-backup create <CLUSTER_ID>
```

```bash title="List available backups"
{{ cliname }} db-backup list <CLUSTER_ID>
```

```bash title="Show the status of the running backup system"
{{ cliname }} db-backup status
```
...
{{ cliname }} db-backup restore <BACKUP_NAME> <CLUSTER_ID>
```
