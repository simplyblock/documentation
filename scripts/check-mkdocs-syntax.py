...
def main():
    ...
    if errors:
        print("mkdocs syntax check failed. Found problems:", file=sys.stderr)
        for v in errors:
            print(
    ...
    # Warnings are grouped per file: they are numerous and every line number is
    # listed, so the report stays readable without dropping anything.
    if warnings:
        grouped = {}
        for v in warnings:
            grouped.setdefault((v.file, v.check), []).append(v.line)
        print(f"\nWarnings ({len(warnings)}), these do not fail the check yet:")
        for (file, check), lines in grouped.items():
            numbers = ", ".join(str(line) for line in lines)
            print(f"  • {file} | {check} | line(s) {numbers}")
        sys.stdout.flush()
    ...
    print(
        f"No mkdocs syntax errors found in {len(files)} file(s) "
        f"({len(placeholders)} placeholder(s) declared, {len(warnings)} warning(s))."
    )
