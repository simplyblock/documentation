...
source: "https://docs.simplyblock.io/latest/non-kubernetes/operations/cluster/failure-domains/"
---
...
sbctl cluster create --enable-failure-domain <FURTHER_OPTIONS>
```

The same flag exists on `sbctl cluster add` when attaching an additional cluster to an existing control
plane.
...
sbctl cluster update <CLUSTER_ID>
```
...
1. Remove the node with `sbctl storage-node remove`.
