...
source: "https://docs.simplyblock.io/latest/non-kubernetes/operations/volumes/volume-migration/"
---
...
sbctl volume migrate <VOLUME_ID> <TARGET_NODE_ID>
```
...
sbctl volume migrate-continue <MIGRATION_ID>
```
...
sbctl volume migrate-list [--cluster-id <CLUSTER_ID>] [--json]
```
...
`sbctl volume get <VOLUME_ID>`.

## Canceling a Migration

```bash title="Cancel a migration"
sbctl volume migrate-cancel <MIGRATION_ID>
```
...
sbctl volume migrate <ANY_MEMBER_VOLUME_ID> <TARGET_NODE_ID> --batch
sbctl volume migrate-continue <GROUP_ID> --batch
sbctl volume migrate-group-list [--cluster-id <CLUSTER_ID>]
sbctl volume migrate-cancel <GROUP_ID> --batch
```
...
(`sbctl storage-node remove` requires the node to host no volumes or snapshots). Migrate all volumes off
