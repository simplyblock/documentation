...
source: "https://docs.simplyblock.io/latest/non-kubernetes/installation/storage-network-multipathing/"
---

# Storage Network Multipathing
...
sbctl storage-node add-node <CLUSTER_ID> <SN_CTR_ADDR> <MGT_IF> \
...
With multipathing, `sbctl volume connect` returns one `nvme connect` command per combination of node and
...
sbctl volume connect <VOLUME_ID>
```
...
1. `sbctl storage-node list --json`: every node reports all of its data interfaces (`data_nics`).
2. `sbctl storage-node port-list <NODE_ID>`: lists the data interfaces of a node.
3. `sbctl volume connect <VOLUME_ID>`: returns one connection string per node and interface (for
   example, four entries for a volume with one failover path on dual-interface nodes).
4. `sbctl storage-node check <NODE_ID>`: verifies all NVMe-oF connections to and from the node,
   including all paths of the cluster-internal connections.

Per-interface I/O statistics are available with `sbctl storage-node port-io-stats <PORT_ID>`.
