...
source: "https://docs.simplyblock.io/latest/non-kubernetes/operations/data-protection/asynchronous-replication/"
---
...
- **Both clusters are managed by the same control plane.** The first cluster is created with
  `sbctl cluster create`. The second is attached to the same control plane with `sbctl cluster add`.
...
sbctl cluster add-replication <SOURCE_CLUSTER_ID> <TARGET_CLUSTER_ID> \
...
sbctl volume replication-start <VOLUME_ID> \
...
A volume can also opt into replication at creation time with `sbctl volume add ... --replicate`, using
...
sbctl volume replication-trigger <VOLUME_ID>
```

## Monitoring

```bash title="Replication progress of one volume"
sbctl volume replication-info <VOLUME_ID>
```
...
sbctl volume replication-status <CLUSTER_ID>
```
...
Wait until `sbctl volume replication-info <VOLUME_ID>` reports an outstanding count of zero, then:

```bash title="Step 2: Commit the cutover"
sbctl volume replication-commit <VOLUME_ID>
```
...
sbctl volume replication-failback <VOLUME_ID> [--source-cluster-id <CLUSTER_ID>]
```
...
with `sbctl volume replication-commit <VOLUME_ID>`. The same client-connect rule applies for an
interruption-free cutback.

## Stopping Replication and Cleaning Up

```bash title="Stop replication for a volume"
sbctl volume replication-stop <VOLUME_ID>
```
...
sbctl snapshot delete-replication-only <SNAPSHOT_ID>
```

The replication status of individual snapshots is available with
`sbctl snapshot replication-status <CLUSTER_ID>` and `sbctl snapshot list --with-details`.
