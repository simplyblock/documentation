...
source: "https://docs.simplyblock.io/latest/non-kubernetes/usage/removing/"
---

# Removing a Logical Volume

Removing a logical Volume (LV) in simplyblock permanently deletes the volume and its associated data from the cluster.
This operation is performed using the `sbctl` command line interface. Care should be taken to verify
...
- A running simplyblock cluster with `sbctl` configured.
...
```bash
sbctl volume delete <VOLUME_UUID> [--force]
```
