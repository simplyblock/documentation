...
source: "https://docs.simplyblock.io/latest/reference/api/alerts/"
---
...
- A node stopped with `sbctl storage-node shutdown` is marked as deliberately stopped until it comes back
  online. Neither the node nor the cluster degradation it causes raises an alert.
- A device removed with `sbctl storage-node remove-device` is marked as administratively removed.
...
sbctl cluster event-alerts <CLUSTER_UUID> --enable
```
